...
guessHandler (puzzle, Guessed) n =
  putStrLn "You guessed that"
    >> pure (puzzle, n)
guessHandler (puzzle, NotGuessed) n =
  putStrLn "Try again"
    >> pure (puzzle, n - 1)
guessHandler (puzzle, AlreadyGuessed) n =
...
runGame' puzzle counter = do
  ...
  case guess of
    [c] ->
      guessHandler (makeGuess puzzle c) counter
        >>= uncurry runGame'
    _ ->
      putStrLn
        "Your guess must\
