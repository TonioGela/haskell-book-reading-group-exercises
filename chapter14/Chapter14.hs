...
import Chapter13 (Puzzle (..), fillInCharacter, handleGuess)
...
propCapitalizeWord st =
  (capitalizeWord st == twice capitalizeWord st)
    && (capitalizeWord st == fourTimes capitalizeWord st)

propSortIdempotence :: Ord a => [a] -> Bool
propSortIdempotence xs =
